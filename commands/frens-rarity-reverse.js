...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('frens-rarity-reverse')
        .setDescription('Flaremingo Frens rarity checker by rank')
        .addStringOption((option) =>
            option
                .setName("rank")
                .setDescription("What rank (rarity) do you want to check?")
                .setRequired(true)
        ),
    async execute(interaction) {
        ...
        if (rank > 0 && rank < 889) {
            ...
            const stmt7 = db.prepare('SELECT edition, rank, image FROM frens WHERE rank = ?')
            var results7 = stmt7.all(rank)
            
            //console.log(results7)

            var nftNumber = results7[0].edition
            ...
            .setTitle(`Flavio here, need anything else?`)
            //.setAuthor({ name: client.user.username })
            .setDescription(`There are only 888 Flaremingo Frens in the wild, try a rank (rarity) between 1 and 888.`)
            .setThumbnail(client.user.avatarURL())
            //.addFields(
            //    { name: 'Your share (wFLR):', value: `${myFlrShare}`},
            //    { name: 'Your share (USD):', value: `${myUsdShare}`},
