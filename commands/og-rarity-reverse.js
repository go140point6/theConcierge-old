...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('og-rarity-reverse')
        .setDescription('Flaremingos rarity checker by rank')
    ...
    async execute(interaction) {
        ...
        if (rank > 0 && rank < 889) {
            ...
            const stmt7 = db.prepare('SELECT edition, rank, image FROM og WHERE rank = ?')
            ...
            .setTitle(`Flavio here, need anything else?`)
            //.setAuthor({ name: client.user.username })
            .setDescription(`There are only 888 Flaremingo in the wild, try a rank (rarity) between 1 and 888.`)
            .setThumbnail(client.user.avatarURL())
            //.addFields(
            //    { name: 'Your share (wFLR):', value: `${myFlrShare}`},
            //    { name: 'Your share (USD):', value: `${myUsdShare}`},
